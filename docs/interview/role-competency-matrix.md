...
| Geometric discrepancy | Map and agent geometry are represented; no geometry metrics are evaluated | Partial | Map/route/overlap metrics, coordinate invariance, and Waymax metric cross-checks in M5–M7 |
...
| Research and experimental rigor | Strong prior research evidence plus an M4 pre-registration, classified population accounting, repeated failure/correction record, bounded empirical acceptance, and explicit claim limitations | Partial | M5 uncertainty and negative-result analysis, then ablations, OOD analysis, and evaluator falsification in M7–M9 |
