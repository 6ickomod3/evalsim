...
|---|:---:|---|---|
| Python | Verified | Python package, contracts, sources, policies, rollout engine, and tests; the accepted official-runner boundary brings the full repository suite to 862 passing tests with 1 expected local-data skip. Its focused evidence includes 14 runner tests, a public mocked 128-case lifecycle, 18 injected failure boundaries, and 34 adversarial lifecycle cases. A fresh environment without JAX, jaxlib, TensorFlow, Flax, or Waymax reports 779 passed with 25 expected optional/local skips | Keep the claim scoped to implemented work; test count is software evidence, not realism evidence |
...
| M5 official-runner implementation | Verified | The accepted streaming runner consumes the explicit accepted 128-case M4 cohort once, separates three policy roles from the Waymax exact-log reference, writes pre-metric parity-order and post-evaluation determinism receipts, and fail-closes source/Git/shard/M4/output/terminal/finalization drift. A public mocked 128-case lifecycle enforces exact domains of 6,656 metric, 1,024 slice, 312 scorecard, and 144 parity-summary rows. Fourteen runner tests, 18 injected failure boundaries, 34 adversarial lifecycle cases, the full suite with 862 passing tests and 1 expected local-data skip, and final adversarial **ACCEPT** support the implementation | Keep this claim scoped to runner software and mocked lifecycle evidence. No M5 WOMD outcome or native WOMD parity result has been computed; the bound official execution and result reviews remain required for any empirical M5 claim |
