...
|---|:---:|---|---|
| Python | Verified | Python package, contracts, sources, policies, rollout engine, and tests; the reviewed official-runner boundary plus its private-repository authentication amendment bring the full repository suite to 869 passing tests with 1 expected local-data skip. Its focused evidence includes 14 runner tests, a public mocked 128-case lifecycle, 18 injected failure boundaries, 34 adversarial lifecycle cases, and 7 private-authentication regression cases. A fresh environment without JAX, jaxlib, TensorFlow, Flax, or Waymax reports 779 passed with 25 expected optional/local skips | Keep the claim scoped to implemented work; test count is software evidence, not realism evidence |
