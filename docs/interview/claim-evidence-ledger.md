...
| Kinematic motion metrics | Not yet | Metric contract only | M5 implementation, analytic oracles, fixed-cohort results, units/eligibility, and Waymax cross-check |
| Interaction metrics | Not yet | No implementation | M5 overlapping-target-frame, center-distance, and capped-TTC-proxy implementation and validation; headway/reactivity move to M6 |
| Map-context diagnostics | Not yet | Retained 2-D lane geometry exists; no evaluator | M5 lane-distance/heading diagnostics and coordinate invariance; true offroad/route semantics require typed M6 context |
| Behavioral slices | Not yet | Synthetic tags exist | Pre-registered WOMD slices, sample counts, missingness, and robustness in M5 |
| Paired finite-cohort stability analysis | Not yet | No statistical implementation | Exact paired effects plus deterministic scenario-reweighting stability bands and edge-case tests in M5; do not call them WOMD-population confidence intervals |
