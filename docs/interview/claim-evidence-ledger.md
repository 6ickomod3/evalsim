...
|---|:---:|---|---|
| Python | Verified | Python package, contracts, sources, policies, rollout engine, and tests; 264 M5-focused tests pass, the locked Waymo-extra suite reports 757 passed with 1 expected local-data skip, and the clean locked core-only suite reports 676 passed with 23 expected optional-runtime skips | Keep the claim scoped to implemented work; test count is software evidence, not realism evidence |
...
| Reproducible evaluation platform | Partial | Deterministic seeds, typed manifests, lossless Parquet, immutable scenario manifests, and an accepted data-free end-to-end command with source binding, exact row derivation, failure preservation, and re-verification | Commit the data-free path, complete the bound real-WOMD M5 acceptance, then add M10 config/cache/resume evidence |
