...
| Reproducible evaluation platform | Partial | Deterministic seeds, typed manifests, lossless Parquet, immutable scenario manifests, and a committed data-free end-to-end command with source binding, exact row derivation, failure preservation, and re-verification | Complete the bound real-WOMD M5 acceptance, then add M10 config/cache/resume evidence |
